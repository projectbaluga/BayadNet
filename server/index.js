...
passport.use(new FacebookStrategy({
    ...
    callbackURL: "https://mgt.bojex.online/auth/facebook/callback",
  ...
  async (accessToken, refreshToken, profile, done) => {
    try {
      ...
      if (user) {
        ...
        // Log the denied attempt for debugging
        console.log(`Access denied for email: ${email}`);
        return done(null, false, { message: 'Access denied: Admin email not recognized.' });
      }
    } catch (err) {
      return done(err);
    }
...
app.get('/auth/facebook/callback',
  passport.authenticate('facebook', { failureRedirect: '/?error=unauthorized', session: false }),
  (req, res) => {
